  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.colors.background }]} edges={['top']}>
      <View style={styles.header}>
        ...
        <Text style={[styles.headerTitle, { color: theme.colors.text }]}>Saved Streams</Text>
        <View style={{ width: 24 }} />
      </View>

      <ScrollView style={styles.scrollView} contentContainerStyle={styles.contentContainer}>
        <View style={styles.emptyState}>
          <IconSymbol ios_icon_name="bookmark.fill" android_material_icon_name="bookmark" size={80} color="#333" />
          <Text style={[styles.emptyStateText, { color: theme.dark ? '#666' : '#999' }]}>
            No saved streams
          </Text>
          <Text style={[styles.emptyStateSubtext, { color: theme.dark ? '#555' : '#aaa' }]}>
            Save streams to watch them later
          </Text>
        </View>
      </ScrollView>
    </SafeAreaView>
