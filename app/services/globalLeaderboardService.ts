
import { supabase } from '@/app/integrations/supabase/client';

export interface CreatorLeaderboardEntry {
  id: string;
  creator_id: string;
  week_start_date: string;
  total_watch_hours: number;
  total_gifts_received_sek: number;
  followers_gained: number;
  total_streams: number;
  composite_score: number;
  rank: number;
  creator?: {
    id: string;
    username: string;
    display_name: string;
    avatar_url: string;
  };
}

export interface FanLeaderboardEntry {
  id: string;
  fan_id: string;
  week_start_date: string;
  total_gift_spending_sek: number;
  total_watch_time_seconds: number;
  comment_activity_count: number;
  is_vip_member: boolean;
  composite_score: number;
  rank: number;
  fan?: {
    id: string;
    username: string;
    display_name: string;
    avatar_url: string;
  };
}

export interface TrendingCreatorEntry {
  id: string;
  creator_id: string;
  week_start_date: string;
  follower_growth_percentage: number;
  viewer_peak_delta: number;
  retention_growth_percentage: number;
  composite_score: number;
  rank: number;
  creator?: {
    id: string;
    username: string;
    display_name: string;
    avatar_url: string;
  };
}

class GlobalLeaderboardService {
  /**
   * Get the start of the current week (Sunday)
   */
  private getCurrentWeekStart(): Date {
    const now = new Date();
    const day = now.getDay();
    const diff = now.getDate() - day;
    const weekStart = new Date(now.setDate(diff));
    weekStart.setHours(0, 0, 0, 0);
    return weekStart;
  }

  /**
   * Get top creators leaderboard
   */
  async getTopCreators(limit: number = 50): Promise<CreatorLeaderboardEntry[]> {
    try {
      const weekStart = this.getCurrentWeekStart().toISOString().split('T')[0];

      const { data, error } = await supabase
        .from('global_leaderboard_creators')
        .select(`
          *,
          creator:profiles!global_leaderboard_creators_creator_id_fkey(
            id,
            username,
            display_name,
            avatar_url
          )
        `)
        .eq('week_start_date', weekStart)
        .order('rank', { ascending: true })
        .limit(limit);

      if (error) {
        console.error('Error fetching top creators:', error);
        return [];
      }

      return data as CreatorLeaderboardEntry[];
    } catch (error) {
      console.error('Error in getTopCreators:', error);
      return [];
    }
  }

  /**
   * Get top fans leaderboard
   */
  async getTopFans(limit: number = 50): Promise<FanLeaderboardEntry[]> {
    try {
      const weekStart = this.getCurrentWeekStart().toISOString().split('T')[0];

      const { data, error } = await supabase
        .from('global_leaderboard_fans')
        .select(`
          *,
          fan:profiles!global_leaderboard_fans_fan_id_fkey(
            id,
            username,
            display_name,
            avatar_url
          )
        `)
        .eq('week_start_date', weekStart)
        .order('rank', { ascending: true })
        .limit(limit);

      if (error) {
        console.error('Error fetching top fans:', error);
        return [];
      }

      return data as FanLeaderboardEntry[];
    } catch (error) {
      console.error('Error in getTopFans:', error);
      return [];
    }
  }

  /**
   * Get trending creators leaderboard
   */
  async getTrendingCreators(limit: number = 50): Promise<TrendingCreatorEntry[]> {
    try {
      const weekStart = this.getCurrentWeekStart().toISOString().split('T')[0];

      const { data, error } = await supabase
        .from('trending_creators')
        .select(`
          *,
          creator:profiles!trending_creators_creator_id_fkey(
            id,
            username,
            display_name,
            avatar_url
          )
        `)
        .eq('week_start_date', weekStart)
        .order('rank', { ascending: true })
        .limit(limit);

      if (error) {
        console.error('Error fetching trending creators:', error);
        return [];
      }

      return data as TrendingCreatorEntry[];
    } catch (error) {
      console.error('Error in getTrendingCreators:', error);
      return [];
    }
  }

  /**
   * Get user's rank in a specific leaderboard
   */
  async getUserRank(
    userId: string,
    leaderboardType: 'top_creators' | 'top_fans' | 'trending_creators'
  ): Promise<number | null> {
    try {
      const weekStart = this.getCurrentWeekStart().toISOString().split('T')[0];
      let tableName: string;
      let columnName: string;

      switch (leaderboardType) {
        case 'top_creators':
          tableName = 'global_leaderboard_creators';
          columnName = 'creator_id';
          break;
        case 'top_fans':
          tableName = 'global_leaderboard_fans';
          columnName = 'fan_id';
          break;
        case 'trending_creators':
          tableName = 'trending_creators';
          columnName = 'creator_id';
          break;
      }

      const { data, error } = await supabase
        .from(tableName)
        .select('rank')
        .eq(columnName, userId)
        .eq('week_start_date', weekStart)
        .single();

      if (error && error.code !== 'PGRST116') {
        console.error('Error fetching user rank:', error);
        return null;
      }

      return data?.rank || null;
    } catch (error) {
      console.error('Error in getUserRank:', error);
      return null;
    }
  }

  /**
   * Get user's leaderboard history
   */
  async getUserHistory(
    userId: string,
    leaderboardType: 'top_creators' | 'top_fans' | 'trending_creators',
    limit: number = 10
  ): Promise<any[]> {
    try {
      const { data, error } = await supabase
        .from('leaderboard_history')
        .select('*')
        .eq('user_id', userId)
        .eq('leaderboard_type', leaderboardType)
        .order('week_start_date', { ascending: false })
        .limit(limit);

      if (error) {
        console.error('Error fetching user history:', error);
        return [];
      }

      return data || [];
    } catch (error) {
      console.error('Error in getUserHistory:', error);
      return [];
    }
  }

  /**
   * Calculate and update leaderboards (should be run weekly)
   */
  async calculateLeaderboards(): Promise<{ success: boolean; error?: string }> {
    try {
      const weekStart = this.getCurrentWeekStart().toISOString().split('T')[0];
      const weekStartDate = this.getCurrentWeekStart();
      const weekEndDate = new Date(weekStartDate);
      weekEndDate.setDate(weekEndDate.getDate() + 7);

      console.log('📊 Calculating leaderboards for week:', weekStart);

      // Calculate Top Creators
      await this.calculateTopCreators(weekStart, weekStartDate, weekEndDate);

      // Calculate Top Fans
      await this.calculateTopFans(weekStart, weekStartDate, weekEndDate);

      // Calculate Trending Creators
      await this.calculateTrendingCreators(weekStart, weekStartDate, weekEndDate);

      console.log('✅ Leaderboards calculated successfully');
      return { success: true };
    } catch (error) {
      console.error('Error in calculateLeaderboards:', error);
      return { success: false, error: 'Failed to calculate leaderboards' };
    }
  }

  /**
   * Calculate top creators leaderboard
   */
  private async calculateTopCreators(
    weekStart: string,
    weekStartDate: Date,
    weekEndDate: Date
  ): Promise<void> {
    try {
      // Get all creators with activity this week
      const { data: streams, error: streamsError } = await supabase
        .from('live_streams')
        .select('creator_id, stream_duration_s, viewer_peak')
        .gte('started_at', weekStartDate.toISOString())
        .lt('started_at', weekEndDate.toISOString())
        .not('ended_at', 'is', null);

      if (streamsError) {
        console.error('Error fetching streams:', streamsError);
        return;
      }

      // Get gift transactions this week
      const { data: gifts, error: giftsError } = await supabase
        .from('gift_transactions')
        .select('receiver_id, amount')
        .gte('created_at', weekStartDate.toISOString())
        .lt('created_at', weekEndDate.toISOString());

      if (giftsError) {
        console.error('Error fetching gifts:', giftsError);
        return;
      }

      // Get followers gained this week
      const { data: followers, error: followersError } = await supabase
        .from('followers')
        .select('following_id')
        .gte('created_at', weekStartDate.toISOString())
        .lt('created_at', weekEndDate.toISOString());

      if (followersError) {
        console.error('Error fetching followers:', followersError);
        return;
      }

      // Aggregate data by creator
      const creatorData = new Map<string, any>();

      streams?.forEach((stream: any) => {
        const existing = creatorData.get(stream.creator_id) || {
          total_watch_hours: 0,
          total_gifts_received_sek: 0,
          followers_gained: 0,
          total_streams: 0,
        };
        existing.total_watch_hours += stream.stream_duration_s / 3600;
        existing.total_streams += 1;
        creatorData.set(stream.creator_id, existing);
      });

      gifts?.forEach((gift: any) => {
        const existing = creatorData.get(gift.receiver_id) || {
          total_watch_hours: 0,
          total_gifts_received_sek: 0,
          followers_gained: 0,
          total_streams: 0,
        };
        existing.total_gifts_received_sek += gift.amount;
        creatorData.set(gift.receiver_id, existing);
      });

      followers?.forEach((follower: any) => {
        const existing = creatorData.get(follower.following_id) || {
          total_watch_hours: 0,
          total_gifts_received_sek: 0,
          followers_gained: 0,
          total_streams: 0,
        };
        existing.followers_gained += 1;
        creatorData.set(follower.following_id, existing);
      });

      // Calculate composite scores and rank
      const entries = Array.from(creatorData.entries()).map(([creatorId, data]) => {
        const compositeScore =
          data.total_watch_hours * 10 +
          data.total_gifts_received_sek * 0.5 +
          data.followers_gained * 5 +
          data.total_streams * 20;

        return {
          creator_id: creatorId,
          week_start_date: weekStart,
          ...data,
          composite_score: compositeScore,
        };
      });

      // Sort by composite score
      entries.sort((a, b) => b.composite_score - a.composite_score);

      // Assign ranks
      entries.forEach((entry, index) => {
        (entry as any).rank = index + 1;
      });

      // Save to history before updating
      for (const entry of entries) {
        await supabase.from('leaderboard_history').insert({
          user_id: entry.creator_id,
          leaderboard_type: 'top_creators',
          week_start_date: weekStart,
          rank: (entry as any).rank,
          composite_score: entry.composite_score,
          metadata: {
            total_watch_hours: entry.total_watch_hours,
            total_gifts_received_sek: entry.total_gifts_received_sek,
            followers_gained: entry.followers_gained,
            total_streams: entry.total_streams,
          },
        });
      }

      // Upsert to leaderboard table
      if (entries.length > 0) {
        await supabase.from('global_leaderboard_creators').upsert(entries, {
          onConflict: 'creator_id,week_start_date',
        });
      }

      console.log(`✅ Top creators calculated: ${entries.length} entries`);
    } catch (error) {
      console.error('Error in calculateTopCreators:', error);
    }
  }

  /**
   * Calculate top fans leaderboard
   */
  private async calculateTopFans(
    weekStart: string,
    weekStartDate: Date,
    weekEndDate: Date
  ): Promise<void> {
    try {
      // Get gift spending this week
      const { data: gifts, error: giftsError } = await supabase
        .from('gift_transactions')
        .select('sender_id, amount')
        .gte('created_at', weekStartDate.toISOString())
        .lt('created_at', weekEndDate.toISOString());

      if (giftsError) {
        console.error('Error fetching gifts:', giftsError);
        return;
      }

      // Get watch time this week
      const { data: views, error: viewsError } = await supabase
        .from('stream_viewers')
        .select('user_id, joined_at, left_at')
        .gte('joined_at', weekStartDate.toISOString())
        .lt('joined_at', weekEndDate.toISOString())
        .not('left_at', 'is', null);

      if (viewsError) {
        console.error('Error fetching views:', viewsError);
        return;
      }

      // Get comment activity this week
      const { data: comments, error: commentsError } = await supabase
        .from('live_comments')
        .select('user_id')
        .gte('created_at', weekStartDate.toISOString())
        .lt('created_at', weekEndDate.toISOString());

      if (commentsError) {
        console.error('Error fetching comments:', commentsError);
        return;
      }

      // Get VIP memberships
      const { data: vips, error: vipsError } = await supabase
        .from('creator_club_memberships')
        .select('member_id')
        .eq('is_active', true);

      if (vipsError) {
        console.error('Error fetching VIPs:', vipsError);
        return;
      }

      const vipSet = new Set(vips?.map((v: any) => v.member_id) || []);

      // Aggregate data by fan
      const fanData = new Map<string, any>();

      gifts?.forEach((gift: any) => {
        const existing = fanData.get(gift.sender_id) || {
          total_gift_spending_sek: 0,
          total_watch_time_seconds: 0,
          comment_activity_count: 0,
          is_vip_member: false,
        };
        existing.total_gift_spending_sek += gift.amount;
        fanData.set(gift.sender_id, existing);
      });

      views?.forEach((view: any) => {
        const watchTime =
          (new Date(view.left_at).getTime() - new Date(view.joined_at).getTime()) / 1000;
        const existing = fanData.get(view.user_id) || {
          total_gift_spending_sek: 0,
          total_watch_time_seconds: 0,
          comment_activity_count: 0,
          is_vip_member: false,
        };
        existing.total_watch_time_seconds += watchTime;
        fanData.set(view.user_id, existing);
      });

      comments?.forEach((comment: any) => {
        const existing = fanData.get(comment.user_id) || {
          total_gift_spending_sek: 0,
          total_watch_time_seconds: 0,
          comment_activity_count: 0,
          is_vip_member: false,
        };
        existing.comment_activity_count += 1;
        fanData.set(comment.user_id, existing);
      });

      // Calculate composite scores and rank
      const entries = Array.from(fanData.entries()).map(([fanId, data]) => {
        const isVip = vipSet.has(fanId);
        const compositeScore =
          data.total_gift_spending_sek * 1.0 +
          (data.total_watch_time_seconds / 3600) * 5 +
          data.comment_activity_count * 2 +
          (isVip ? 100 : 0);

        return {
          fan_id: fanId,
          week_start_date: weekStart,
          ...data,
          is_vip_member: isVip,
          composite_score: compositeScore,
        };
      });

      // Sort by composite score
      entries.sort((a, b) => b.composite_score - a.composite_score);

      // Assign ranks
      entries.forEach((entry, index) => {
        (entry as any).rank = index + 1;
      });

      // Save to history
      for (const entry of entries) {
        await supabase.from('leaderboard_history').insert({
          user_id: entry.fan_id,
          leaderboard_type: 'top_fans',
          week_start_date: weekStart,
          rank: (entry as any).rank,
          composite_score: entry.composite_score,
          metadata: {
            total_gift_spending_sek: entry.total_gift_spending_sek,
            total_watch_time_seconds: entry.total_watch_time_seconds,
            comment_activity_count: entry.comment_activity_count,
            is_vip_member: entry.is_vip_member,
          },
        });
      }

      // Upsert to leaderboard table
      if (entries.length > 0) {
        await supabase.from('global_leaderboard_fans').upsert(entries, {
          onConflict: 'fan_id,week_start_date',
        });
      }

      console.log(`✅ Top fans calculated: ${entries.length} entries`);
    } catch (error) {
      console.error('Error in calculateTopFans:', error);
    }
  }

  /**
   * Calculate trending creators leaderboard
   */
  private async calculateTrendingCreators(
    weekStart: string,
    weekStartDate: Date,
    weekEndDate: Date
  ): Promise<void> {
    try {
      // Get previous week dates
      const prevWeekStart = new Date(weekStartDate);
      prevWeekStart.setDate(prevWeekStart.getDate() - 7);
      const prevWeekEnd = new Date(weekStartDate);

      // Get current week followers
      const { data: currentFollowers, error: currentError } = await supabase
        .from('followers')
        .select('following_id')
        .gte('created_at', weekStartDate.toISOString())
        .lt('created_at', weekEndDate.toISOString());

      if (currentError) {
        console.error('Error fetching current followers:', currentError);
        return;
      }

      // Get previous week followers
      const { data: prevFollowers, error: prevError } = await supabase
        .from('followers')
        .select('following_id')
        .gte('created_at', prevWeekStart.toISOString())
        .lt('created_at', prevWeekEnd.toISOString());

      if (prevError) {
        console.error('Error fetching previous followers:', prevError);
        return;
      }

      // Calculate follower growth
      const currentCounts = new Map<string, number>();
      const prevCounts = new Map<string, number>();

      currentFollowers?.forEach((f: any) => {
        currentCounts.set(f.following_id, (currentCounts.get(f.following_id) || 0) + 1);
      });

      prevFollowers?.forEach((f: any) => {
        prevCounts.set(f.following_id, (prevCounts.get(f.following_id) || 0) + 1);
      });

      const entries: any[] = [];

      currentCounts.forEach((currentCount, creatorId) => {
        const prevCount = prevCounts.get(creatorId) || 0;
        const growthPercentage =
          prevCount > 0 ? ((currentCount - prevCount) / prevCount) * 100 : currentCount * 100;

        // Simple composite score based on growth
        const compositeScore = growthPercentage;

        entries.push({
          creator_id: creatorId,
          week_start_date: weekStart,
          follower_growth_percentage: growthPercentage,
          viewer_peak_delta: 0, // Would need more complex calculation
          retention_growth_percentage: 0, // Would need more complex calculation
          composite_score: compositeScore,
        });
      });

      // Sort by composite score
      entries.sort((a, b) => b.composite_score - a.composite_score);

      // Assign ranks
      entries.forEach((entry, index) => {
        entry.rank = index + 1;
      });

      // Save to history
      for (const entry of entries) {
        await supabase.from('leaderboard_history').insert({
          user_id: entry.creator_id,
          leaderboard_type: 'trending_creators',
          week_start_date: weekStart,
          rank: entry.rank,
          composite_score: entry.composite_score,
          metadata: {
            follower_growth_percentage: entry.follower_growth_percentage,
            viewer_peak_delta: entry.viewer_peak_delta,
            retention_growth_percentage: entry.retention_growth_percentage,
          },
        });
      }

      // Upsert to leaderboard table
      if (entries.length > 0) {
        await supabase.from('trending_creators').upsert(entries, {
          onConflict: 'creator_id,week_start_date',
        });
      }

      console.log(`✅ Trending creators calculated: ${entries.length} entries`);
    } catch (error) {
      console.error('Error in calculateTrendingCreators:', error);
    }
  }
}

export const globalLeaderboardService = new GlobalLeaderboardService();